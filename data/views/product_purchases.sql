...
select count(*) entries, oi.sku, oi.product_id, oi.name,
    ...
    (case
      when cp.visibility = 1 then 'Not Visible'
      when cp.visibility = 2 then 'Catalog'
      when cp.visibility = 3 then 'Search'
      when cp.visibility = 4 then 'Catalog/Search'
    end) visibility,
  ...
  where
    ...
    ccev.attribute_id = 33
  group by
    oi.sku, oi.product_id, oi.name, ps.sup_name, cast(oi.created_at as date),
    attribute_set_name, cpi.price, cpi.min_price, oi.price, oi.discount_amount,
    cp.visibility, st.value, oi.product_type
  order by
